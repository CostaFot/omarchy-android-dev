...
Service.qml                  Item, one per shell: hosts registry, Store, the tracker Process (`track` + SplitParser, backoff), device notifications, IpcHandler target costafot.android-dev
Store.qml                    QtObject: runs the helper (one at a time, last command wins, tripwire, budget + kill timers, runGen), holds adb/devices/selected/packages/toggles/status, notice
...
Panel.qml                    kit Panel (manageIpc false) > KeyboardPanel > PanelKeyCatcher > Flickable > Repeater(rows); page stack; hub renderer; row kinds title header sep action note footer
...
- **Store.qml** is Markets' Store (`run(args, onDone)`, one `Process` through `/bin/sh -c 'exec "$0" "$@"' /usr/bin/python3`, last-command-wins `pendingRun`, `collectorDone`+`processDone` with the 300 ms `exitFallback`, 1 MiB `StdioCollector` tripwire, `helperBudgetSeconds` 60 as `OMARCHY_ANDROID_DEV_TOTAL_BUDGET` through `proc.environment` assigned in `run()`, `killTimer` SIGTERM 10 s after the budget then `killFallback` SIGKILL 5 s later) plus **`runGen`**: every run increments it and each timer carries the generation it was armed for, so a stale timer from a killed run never fires into the next one; a `timedOut` run is reported as `timeout` whatever it printed. Merge rule in `handle()`: `adb` → `adbInfo` always; `command:"status"` → `status`; a `devices` array → `devices` + `selected`; `packages` → `packages` + `packagesInfo`; `toggles` → `toggles`; `error` → `lastError`/`lastErrorCode` without blanking anything; `notice` → `showNotice` (3 s), and an `ok:false` error message becomes an urgent notice. `applyTrackerEvent(ev)` is the tracker's way in (no queue). No polling timer anywhere: the tracker pushes devices, the panel refreshes `status` on open and `r`.
...
- **Panel.qml** keeps a page `stack` (`push` saves `cursor`, `pop` restores it, `showPage(name)` for the IPC verb with `pendingStack` when closed), `rows` is one flat array (`title header sep action note footer`) painted by one `Repeater` in one `Flickable`; only `action` rows take the cursor (`hoverRow` ignores synthetic moves). The hub: the device row (label + `detail`, or `No device`, or `Looking for adb…` before the first document, or the `adb not found` note with the `Look again` row; Enter on the device row re-reads status), a tracker or helper error as an urgent note, then the eight page rows muted with `0.x.0 · what they will do`, the notice, the key hint. Keys: `j/k`/arrows move, Enter activates, `r` refreshes, Escape pops (closes on the hub), Backspace pops through a `Keys.onPressed` on the catcher's parent, Tab switches bar panels. Width `fittedContentWidth(Style.space(380))`, height capped at `Style.space(760)`. Every `Text` is `Text.PlainText`.
...
```bash
python3 -m unittest discover -s tests -v            # 88 offline tests, ~11 s; no device, no real adb, no network
...
~~0 survey the marketplace's twelve Android plugins~~ · ~~1 repo docs + Python core (terminal only)~~ (0.1.0) · ~~2 service, bar widget, hub~~ (0.2.0) · 3 devices, packages, actions, deep links (the `deeplink` command already exists; the pages remain) · 4 toggles, screenshot, screen record · 5 APK manager, send text, tools · 6 settings page, IPC, keybinding docs · 7 release polish 1.0.0 · 8 publish (marketplace, projects page, blog draft)

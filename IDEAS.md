...
- **The emulator window on Hyprland.** Seen with the `Medium_Phone` AVD on 2026-09-05: the main window (class `Emulator`, title `Android Emulator - Medium_Phone:5554`) is tiled to whatever the layout gives it (about 1005×1102 here) and the phone screen stays at its own size in the top-left corner with the rest blank; the toolbar is a second floating window (class `Emulator`, title `Emulator`) parked beside it. Not this session's problem: S5 launches AVDs from the Tools page and S7 puts the float-and-center rule for the `Emulator` class in the README (the plan's S0 finding, from ADB Devices). To check then: whether the launch flags the plugin passes can size the window or hide the toolbar, and whether a window rule alone is enough or the scale needs the emulator's own zoom.
